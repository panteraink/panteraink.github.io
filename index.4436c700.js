...
plg.sliderCreate = ({ containerSelector, content, buttonsShow = true, slidesClass, buttonsClass, containerClass, slidesReverseOrder = false })=>{
    ...
    function buttons() {
        ...
        if (buttonsClass) buttonsClass.forEach((c)=>{
    ...
    if (containers.length !== 1) throw new Error("Not unique identificator");
    const container = containers[0];
    if (containerClass) containerClass.forEach((c)=>{
    ...
    const slideNext = ()=>{
        ...
        ++slideNumber;
        --slideCount;
        if (slides[slideNumber + 1]) buttonNext.classList?.remove("inactive-button");
        else if (!slides[slideNumber + 1]) buttonNext.classList?.add("inactive-button");
    };
    ...
    const slidePrevious = ()=>{
        ...
        --slideNumber;
        ++slideCount;
        if (slides[slideNumber - 1]) buttonPrevious.classList?.remove("inactive-button");
        else if (!slides[slideNumber - 1]) buttonPrevious.classList?.add("inactive-button");
    };
    ...
    const sliderTimer = (interval)=>{
        const func = ()=>{
            if (content.length === 1) throw new Error("This slider have only one slide");
            if (slideDirection === "forward") {
                if (slideCount > 1) slideNext();
                else if (slideCount === 1) {
                    slideDirection = "backward";
                    slidePrevious();
                }
            } else if (slideDirection === "backward") {
                if (slideCount < content.length) slidePrevious();
...
//# sourceMappingURL=index.4436c700.js.map
