...
const expandingButtonsActivate = ()=>{
    ...
    container?.addEventListener("click", (event)=>{
        if (event.target.closest(".button-rotating")) event.target.closest(".button-rotating").classList.toggle("rotated");
        else if (event.target.closest(".button-expanding")) {
            const obj = event.target.closest(".button-expanding");
            if (obj.dataset.expandingButton === "closed") expandingButtonExpand(obj);
            else if (obj.dataset.expandingButton === "expanded") expandingButtonCollapse(obj);
        } else if (event.target.closest(".outer-wrapper")) {
            for (const obj of buttonsExpanding)if (obj.dataset.expandingButton === "expanded") expandingButtonCollapse(obj);
        }
    });
};

//# sourceMappingURL=index.add810da.js.map
